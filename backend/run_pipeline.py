"""
run_pipeline.py — DeadSat Resurrection · End-to-End Pipeline
=========================================================
...
-----
  ...
  # Single fault type:
  python run_pipeline.py --fault SEU
  python run_pipeline.py --fault software_bug
  python run_pipeline.py --fault firmware_corruption
  python run_pipeline.py --fault command_injection

  # Skip the classifier (inject fault directly, mock confidence):
  python run_pipeline.py --fault SEU --skip-classifier

  # Use a different satellite from the catalog (default 28654 = NOAA 18):
  python run_pipeline.py --all --norad-id 25544
...
for _p in (ROOT, ROOT / "pipeline", ROOT / "agents", ROOT / "emulator"):
    if str(_p) not in sys.path:
        sys.path.insert(0, str(_p))

from emulator.satellite_emulator import SatelliteEmulator, FaultType  # noqa: E402
from pipeline.classifier_inference import (  # noqa: E402
...
def main() -> int:
    ...
    if not args.fault and not args.all:
        parser.print_help()
        print("\nExample: python run_pipeline.py --all --skip-classifier")
        return 0
